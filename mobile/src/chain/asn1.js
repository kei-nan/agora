/**
 * Verbatim copy of `test/asn1.js` from rarimo/passport-zk-circuits
 * (https://github.com/rarimo/passport-zk-circuits, MIT License,
 * Copyright (c) 2023 Zero Block Global Foundation), itself a fork of Lapo
 * Luchini's asn1js BER/DER decoder (lapo.it/asn1js). Kept byte-identical to
 * the upstream source (not hand-ported) so `decoded()`'s tree shape
 * (`{name, content, dump, header, length, tagLen, sub}`) matches exactly
 * what sodParser.ts's field-extraction logic — itself a port of that repo's
 * `process_passport.js` — expects. Pure JS, no Node-specific APIs, so it
 * runs unmodified under React Native/Metro (`allowJs` is on in
 * tsconfig.json). Do not "clean up" or restyle this file; a byte-diff
 * against upstream should stay meaningful if this ever needs re-syncing.
 */
const
