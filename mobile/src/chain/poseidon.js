/**
 * Verbatim copy of `test/poseidon.js` from rarimo/passport-zk-circuits
 * (https://github.com/rarimo/passport-zk-circuits, MIT License, Copyright
 * (c) 2023 Zero Block Global Foundation) — itself circomlibjs's reference
 * Poseidon permutation (same POSEIDON_C/M/P/S constant tables, same
 * PoseidonEx round structure), vendored here rather than pulled in via the
 * `circomlibjs` npm package because circomlibjs's poseidon build goes
 * through a WASM field-arithmetic backend (ffjavascript/wasmcurves) that
 * isn't guaranteed to run under React Native/Hermes, whereas this file is
 * pure BigInt arithmetic with no WASM, no Node-specific APIs. Kept
 * byte-identical to the upstream source (not hand-ported) so its output is
 * verified against the same file, not a re-typed copy — see
 * certificateTree.test.ts, which cross-checks this implementation's output
 * against a value independently computed by literally requiring this exact
 * upstream file. Do not "clean up" or restyle; a byte-diff against upstream
 * should stay meaningful if this ever needs re-syncing.
 */
