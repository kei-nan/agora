...
//!
//! Mirrors `desktop/src-tauri/src/rpc.rs` in spirit: this project's existing convention for
//! Rust chain connectivity is a small hand-rolled JSON-RPC client (`reqwest` +
//! `state_getStorage`/`state_getKeysPaged`/`state_queryStorageAt`/manual SCALE decoding), not
//! `@polkadot/api` (JS-only) and not `subxt` (a heavier, code-generated client this project has
//! not adopted anywhere in this working tree).
//!
...
//! `submit_extrinsic` (the one write path the desktop app never needs, since it is read-only
//! per CLAUDE.md's "AI Agent is read-only on-chain" — this service is not that agent).
//!
//! One deliberate DIVERGENCE from `desktop/src-tauri/src/rpc.rs`: `twox128_hex` below is NOT
//! copied verbatim. Checked against the well-known `twox128("System") ==
//! "26aa394eea5630e07c48ae0c9558cef7"` reference vector (see the test below), the desktop
//! version's `format!("{:016x}{:016x}", r0.to_le(), r1.to_le())` does NOT reproduce it —
//! `u64::to_le()` is a no-op on any little-endian host *for the numeric value*, so `{:016x}`
//! still prints the value's ordinary big-endian hex digits, byte-reversed relative to the
//! correct answer. This is very likely a real bug in the desktop app's storage-key computation
//! on any real (little-endian) deployment — found independently while writing this file, not
//! copied from anywhere. It is out of this task's scope to fix `desktop/src-tauri/src/rpc.rs`
//! itself (a separate, unrelated component); flagged here and in this crate's README instead.
...
/// Live values needed to build a signed extrinsic (`extrinsic.rs`), fetched fresh per
/// submission rather than cached — this service submits at most a handful of rulings per poll
/// cycle, so there's no meaningful cost to always reading current chain state over risking a
/// stale nonce/spec-version.
...
impl RpcClient {
    ...
    /// both for `CheckGenesis` and (since this service signs `Era::Immortal` transactions, see
    ...
    /// `CheckTxVersion`. Fetched live (not hardcoded) so this service keeps working across a
    /// runtime upgrade without a rebuild.
    ...
    /// transaction use", accounting for transactions already in the pool.
    ...
    /// `author_submitExtrinsic`. Returns the extrinsic hash the node assigned it.
...
/// **little-endian byte representation** (not the hex digits of the numeric value — see this
/// module's header comment for why that distinction matters and what it broke in the desktop
/// app's own copy of this function).
...
mod tests {
    ...
    fn storage_prefix_is_64_hex_chars_plus_0x() {
        let prefix = storage_prefix("Courts", "Cases");
