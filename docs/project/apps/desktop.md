...
- **Chain connectivity (changelog #089): a real embedded smoldot light client, in the JS
  ...
  headlessly — see changelog #089 for the full verification writeup.
...
- **AI agent** (`commands/agent.rs`, frontend in `desktop/src/context/AgentContext.tsx` +
  `desktop/src/components/AgentPanel.tsx`): `agent_ask(question, item_context, history)` — calls
  Claude API (`claude-sonnet-4-6`); reads `CLAUDE_API_KEY` env var; degrades gracefully offline.
  **Scope, as actually implemented: read-only plain-text Q&A only.** `ask()` in
  `AgentContext.tsx` calls `invoke("agent_ask", ...)` and appends the string reply to a message
  list — that's the entire round trip. There is no action-drafting data structure, no suggested-
  delegation (or any other suggested-action) UI, and no call anywhere in the agent code path to
  `chain_submit_extrinsic` or any other write/submit command. CLAUDE.md's "AI Agent Features"
  section currently describes the agent as able to "draft actions (e.g. suggest a delegation)"
  pending user confirm-and-sign on the phone — that description is aspirational/overstated
  against the code as it stands today; this file reflects what's actually built.
...
- **Extrinsic submission**: `chain_submit_extrinsic` is registered and auth-gated — it checks a valid, non-expired bearer token via `auth::require_submit_session` (not the plain `require_valid_session` that gates the read-only commands), which additionally requires `SessionScope::ReadSubmit` before relaying bytes to `author_submitExtrinsic`. No session-minting path in the codebase ever issues `ReadSubmit` today (QR-auth's `finalize_callback` hardcodes `SessionScope::Read`), so this command is currently unreachable by any real session, not merely uncalled. It is also a thin passthrough that expects an already phone-signed extrinsic as input — no frontend file calls it, and the command's own doc comment in `commands/chain.rs` notes there is no phone-side flow yet that produces that signed input (QR-auth today only carries a signed challenge string, not a signed call). Treat it as plumbed but not yet connected to any UI flow, not as a working end-to-end feature like the read commands above.
